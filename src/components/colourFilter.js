...
class ColourTable extends React.Component {
    render() {
        ...
        coloursTemp.forEach((row) => {
            row.forEach((colour) => {
                lowerCaseColour = colour.name.toLowerCase();

                if (lowerCaseColour.indexOf(this.props.filterText.toLowerCase()) !== -1) {
                    coloursFiltered.push(
                        colour
                    );
                } 
            });
        ...
        for (let i = 0; i < coloursFiltered.length; i += 5) {
            colours.push(coloursFiltered.slice(i, i + 5))
        }
